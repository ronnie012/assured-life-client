...
const BlogCard = ({ blog }) => {
  ...
  return (
    <div className="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 flex flex-col justify-between">
      <div>
        <img src={blog.blogImage || 'https://via.placeholder.com/300'} alt={blog.title} className="w-full h-48 object-cover rounded-t-lg mb-4" />
      ...
      <Link to={`/blog/${blog._id}`} className="inline-flex items-center px-3 py-2 mt-4 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 self-start">
        Read more
      </Link>
    </div>
  );
